...
export default async function AdminStoreOwnersPage() {
  const users = await getUsers(50, 1, undefined, 'STORE_OWNER');
  const totalCount = await getUsersCount(undefined, 'STORE_OWNER');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Store Owners Management</h1>
          <p className="text-sm text-slate-500">{totalCount} total store owners in the system.</p>
        </div>
        <Link href="/admin/store-owners/new" className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium">
                ...
                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin/store-owners/${user.id}/edit`} className="p-2 text-slate-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-lg transition-colors">

...
const roleColors: Record<string, string> = {
  ...
  ARTIST: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 border-purple-300 dark:border-purple-700',
...
export function UserRoleSelect({ userId, currentRole }: { userId: string; currentRole: string }) {
  ...
  return (
    <select
      ...
      disabled={isUpdating}
      className={`px-2.5 py-1 rounded-full text-xs font-medium border cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 ${roleColors[role] || roleColors.USER}`}
    >
      {roles.map((r) => (
